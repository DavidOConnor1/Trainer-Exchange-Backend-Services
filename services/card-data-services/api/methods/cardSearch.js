 ...
 async searchCards(searchParams = {}) {
    ...
    try {
        const result = await this.retryHandler.withRetry(async () => {
            ...
            const fullCards = await Promise.all(
                cardResumes.map(async (resume) => {
                    try {
                        ...
                        const pricingResult = await this.pricingMethods.fetchCardWithPricing(fullCard.id);
                        const set = await fullCard.getSet();
                        const serie = set ? await set.getSerie() : null;
                        
                        return {
                            fullCard,
                            pricingData: pricingResult.pricing,  // Store as pricingData
                            hasPricing: pricingResult.hasPricing,
                            relatedSet: set,
                            relatedSerie: serie
                        };
                    } catch (error) {
                        console.log(`  ⚠️ Failed to fetch full details for ${resume.id}: ${error.message}`);
                        return { 
                            fullCard: resume, 
                            pricingData: null,
                            hasPricing: false,
            ...
            const transformedCards = fullCards.map(({ fullCard, pricingData, hasPricing, relatedSet, relatedSerie }) => {
                let setInfo = null;
                if (relatedSet) {
                    setInfo = {
                        id: relatedSet.id,
                ...
                // Build pricing info from pricingData
