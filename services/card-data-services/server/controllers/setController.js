...
export const setController = {
  ...
  getCardsBySet: asyncHandler(async (req, res) => {
    try {
      ...
      const result = await pokemonAPI.getCardsBySet(
        setId,
        parseInt(page),
        parseInt(pageSize),
      );
  ...
  getCardsByType: asyncHandler(async (req, res) => {
    try {
      ...
      const results = await pokemonAPI.getCardsByType(
        type,
        parseInt(page),
        parseInt(pageSize),
      );

      ResponseHandler.success(res, results, `Cards of type ${type} retrieved`);
    } catch (error) {
