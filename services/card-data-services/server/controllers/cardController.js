...
export const cardController = {
  getCardByLocalId: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ getCardByLocalId failed after ${duration}ms:`,
        error.message,
      );
    ...
    try {
      ...
      if (!card) {
        return ResponseHandler.notFound(
          res,
          `Card with set ${setId} and localId ${localId}`,
        );
      }
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ getCardBySetAndLocalId failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
  ...
  getCardBySdkId: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      ResponseHandler.success(
        ...
        {
          warning:
            "This is an internal SDK ID. Use /api/cards/:localId for user-facing queries.",
        },
      );
    } catch (error) {
      const duration = Date.now() - startTime;
      console.error(
        `❌ getCardBySdkId failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
  ...
  getCardPricing: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      console.log(
        `${hasPricing ? "✅" : "⚠️"} Pricing fetched for ${card.name} (${duration}ms) - ${hasPricing ? "Pricing available" : "No pricing data"}`,
      );
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ getCardPricing failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
    ...
    try {
      ...
      if (!cardIds || !Array.isArray(cardIds) || cardIds.length > 50) {
        return ResponseHandler.error(
          res,
          "Invalid request. Maximum 50 cards per batch",
          400,
        );
      }
      ...
      const duration = Date.now() - startTime;
      console.log(
        `✅ Batch complete: ${results.summary?.successful || 0} successful, ${results.summary?.failed || 0} failed (${duration}ms)`,
      );
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ batchGetCards failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
  ...
  getCardPricingByFullId: asyncHandler(async (req, res) => {
    const { fullId } = req.params;
    // Fetch the full card using the existing method
    const card = await pokemonAPI.getCardById(fullId);
    // The card object already contains pricing from the enrich step
