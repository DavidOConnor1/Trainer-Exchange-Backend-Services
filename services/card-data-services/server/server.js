...
if (!envLoaded && process.env.NODE_ENV !== "test") {
...
console.log(
  `✅ JWT_API_KEY_SECRET loaded: ${!!process.env.JWT_API_KEY_SECRET}`,
);
...
// CORS configuration
const allowedOrigins = ["https://trainer-exchange.vercel.app"];
...
async function testTCGdexConnection() {
  ...
  try {
    ...
    if (testCard && testCard.id) {
      ...
      console.log(
        `  📇 Test card found: ${testCard.name} (localId: ${testCard.localId})`,
      );
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      while (randomCards.length < 5 && attempts < maxAttempts) {
        attempts++;
        console.log(
          `[${req.id}]   Fetching random card ${randomCards.length + 1}/5 (attempt ${attempts})...`,
        );
        ...
        if (randomCard && randomCard.id && !usedIds.has(randomCard.id)) {
          ...
          console.log(
            `[${req.id}]     ✅ Got: ${randomCard.name} (${randomCard.localId}) - Trend: ${trendPrice ? `€${trendPrice}` : "N/A"}`,
          );
        } else if (randomCard && usedIds.has(randomCard.id)) {
          console.log(
            `[${req.id}]     ⚠️ Duplicate card: ${randomCard.name} (${randomCard.localId}), fetching another...`,
          );
        }
      }

      console.log(
        `[${req.id}] ✅ Successfully fetched ${randomCards.length}/5 unique random cards`,
      );
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      while (randomCards.length < count && attempts < maxAttempts) {
        attempts++;
        console.log(
          `[${req.id}]   Fetching random card ${randomCards.length + 1}/${count} (attempt ${attempts})...`,
        );
        ...
        if (randomCard && randomCard.id && !usedIds.has(randomCard.id)) {
          ...
          console.log(
            `[${req.id}]     ✅ Got: ${randomCard.name} (${randomCard.localId}) - ${randomCard.set?.name} - Trend: ${trendPrice ? `€${trendPrice}` : "N/A"}`,
          );
        } else if (randomCard && usedIds.has(randomCard.id)) {
          console.log(
            `[${req.id}]     ⚠️ Duplicate card: ${randomCard.name} (${randomCard.localId}), fetching another...`,
      ...
      console.log(
        `[${req.id}] ✅ Successfully fetched ${randomCards.length}/${count} unique random cards`,
      );
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      const tcgdex = new TCGdex("en");
      const query = Query.create().contains("name", "Pikachu").paginate(1, 5);
...
app.get("/api/status", (req, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      successfulCalls: apiCallLog.filter((log) => log.status === "success")
        .length,
...
const shutdown = () => {
  ...
  const successfulRequests = apiCallLog.filter(
    (log) => log.status === "success",
  ).length;
  const failedRequests = apiCallLog.filter(
