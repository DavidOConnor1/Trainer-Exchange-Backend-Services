...
describe("BatchMethods", () => {
  ...
  beforeEach(() => {
    // Mock the tcgdex client with card.get method
    mockTCGdexClient = {
      card: {
        get: jest.fn(),
        list: jest.fn(),
    ...
    // Make withRetry execute the function immediately
  ...
  describe("getCardsBatch", () => {
    it("should fetch multiple cards by SDK IDs successfully", async () => {
      ...
      // Mock the card.get method instead of fetch
      mockTCGdexClient.card.get
        .mockResolvedValueOnce(mockCard1)
        .mockResolvedValueOnce(mockCard2);

      const result = await batchMethods.getCardsBatch([
        "swsh3-136",
        "swsh3-137",
      ]);
    ...
    it("should handle failures when fetching cards", async () => {
      mockTCGdexClient.card.get.mockRejectedValue(new Error("API Error"));
    ...
    it("should use cached cards when available", async () => {
      ...
      // card.get should not be called since cached
      expect(mockTCGdexClient.card.get).not.toHaveBeenCalled();
