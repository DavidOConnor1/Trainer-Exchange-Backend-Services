...
function cleanCardData(card) {
  ...
  if (
    card.pricing &&
    (card.pricing.avg30 || card.pricing.trend || card.pricing.avg)
  ) {
    pricingInfo = {
...
export const searchController = {
  searchCards: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (localId) {
        // If name is also provided, search by name and filter by localId
        if (name) {
          searchParams.name = name;
          searchParams.localId = localId;
        } else {
          // If only localId, we can't do much - might need name or full ID
          searchParams.localId = localId;
        }
      }
      ...
      console.log(
        `✅ Search completed: ${cleanedData.length} results in ${duration}ms`,
      );
  ...
  searchByAbility: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const results =
        (await pokemonAPI.searchByAbility?.(
          text,
          parseInt(page),
          parseInt(pageSize),
        )) || [];
      ...
      console.log(
        `✅ Ability search complete: ${cleanedData.length} results in ${duration}ms`,
      );

      ResponseHandler.success(
        res,
        cleanedData,
        `Found ${cleanedData.length} cards with ability containing "${text}"`,
        200,
        {
          metadata: {
            responseTime: `${duration}ms`,
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ Ability search failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
  ...
  searchByAttack: asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const results =
        (await pokemonAPI.searchByAttack?.(
          name,
          parseInt(page),
          parseInt(pageSize),
        )) || [];
      ...
      console.log(
        `✅ Attack search complete: ${cleanedData.length} results in ${duration}ms`,
      );

      ResponseHandler.success(
        res,
        cleanedData,
        `Found ${cleanedData.length} cards with attack "${name}"`,
        200,
        {
          metadata: {
            responseTime: `${duration}ms`,
      ...
      const duration = Date.now() - startTime;
      console.error(
        `❌ Attack search failed after ${duration}ms:`,
        error.message,
      );
      ResponseHandler.error(res, error);
    }
