...
export class SetMethods {
  ...
  async getAllSets() {
    ...
    try {
      const sets = await this.retryHandler.withRetry(
        async () => {
          // Use fetch('sets') as per documentation
          const allSets = await this.tcgdex.fetch("sets");

          // The fetch('sets') returns an array of set objects
          // Each set already has serie information
  ...
  async getSetById(setId) {
    ...
    try {
      const set = await this.retryHandler.withRetry(
        async () => {
